...
_OPEN_ENDED_FINANCE_RE = re.compile(
    ...
    r"людськ\w*\s+мов|простими\s+словами|human\s+language|"
    r"м'?які\s+висновк|висновк|soft\s+conclusion|"
    r"як\s+коуч|as\s+coach|coach|"
    r"підсумуй|підсумок|сформулюй|опиши|describe|formulate|summari[sz]e|summary|"
    ...
    r"чим\s+.*відрізня\w*\s+від\s+попередн\w*|коротко\s+і\s+по\s+суті|"
    r"де\s+просідаю|що\s+тисне\s+на\s+бюджет|що\s+найбільше\s+тисне|"
    r"що\s+можна\s+підрізати|де\s+можна\s+зекономити|"
...
_SEMANTIC_REASONING_RE = re.compile(
    ...
    r"більше\s+йде\s+на|де\s+йде\s+найбільше|"
    r"куди\s+.*йдуть\s+грош\w*|на\s+що\s+.*йдуть\s+грош\w*|"
    r"що\s+добре|що\s+погано|на\s+що\s+звернути\s+увагу|"
    r"який\s+один\s+.*крок|найреалістичн\w*\s+крок|без\s+сильн\w*\s+дискомфорт\w*|"
    r"чим\s+.*відрізня\w*\s+від\s+попередн\w*|"
    r"коротко\s+і\s+по\s+суті|"
    r"що\s+тисне\s+на\s+бюджет|що\s+найбільше\s+тисне|"
    r"де\s+можна\s+зекономити|"
...
_NARRATIVE_ONLY_RE = re.compile(
    r"\b("
    r"опиши|describe|"
    r"підсумуй|підсумок|сформулюй|formulate|summari[sz]e|summary|"
    r"що\s+це\s+говорить|what\s+does\s+this\s+say|"
    r"людськ\w*\s+мов|простими\s+словами|human\s+language|"
    r"м'?які\s+висновк|висновк|soft\s+conclusion|"
    r"як\s+коуч|as\s+coach|coach|"
    ...
    r"чим\s+.*відрізня\w*\s+від\s+попередн\w*|"
    r"коротко\s+і\s+по\s+суті|"
    r"що\s+тисне\s+на\s+бюджет|де\s+можна\s+зекономити|"
